...
function problemC() {
  ...
  i++
    ...
    caja = caja.then((stanza) =>  {
      ...
      //para validar si llegamos al final hacemos:
...
function problemD() {
  ...
  // promise version
  for(let i = 1, caja = promisifiedReadFile(filenames[0]); i <= filenames.length;i++){
      caja = caja.then((stanza) =>  {
        blue(stanza);
...
function problemE() {
   ...
   *
   */
//--- No está probada la siguiente respuesta al problema:
  var fs = require("fs");
  function promisifiedWriteFile(filename, str) {
    // tu código aquí
    return new Promise(function (resolve, reject) {
      fs.promisifiedWriteFile(filename, str, function (err, str) {
        if (err) reject(err);
        else resolve(str);
      });
    });
